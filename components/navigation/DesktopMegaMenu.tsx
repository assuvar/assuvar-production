...
export default function DesktopMegaMenu() {
    ...
    return (
        <div
            className="hidden md:block" // Ensure desktop only
        ...
        >
            <div className="relative h-20 flex items-center cursor-pointer group">
                <span className={`transition-colors flex items-center gap-1 text-sm font-medium ${isOpen ? 'text-structura-black' : 'text-slate-600 hover:text-structura-black'}`}>
                    What we do <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
                </span>
                    ...
                    {isOpen && (
                        ...
                        `}
                                    ...
                                    {activeCategory.featuredBlog && (
                                        <div className="mt-6">
                                            <FeaturedBlogCard blog={activeCategory.featuredBlog} />
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex-1 p-8 bg-white">
                                    <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-50">
                                        <h4 className="text-xl font-display font-bold text-structura-black">{activeCategory.label}</h4>
                                        <Link
                                    ...
                                    >
                                        {activeCategory.services.map((service) => (
                                            <Link
                                                ...
                                                className="group/link flex flex-col py-2 px-3 -mx-3 rounded-lg hover:bg-slate-50 transition-colors"
                                            >
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="text-sm text-structura-black font-semibold group-hover/link:text-[#6A0DAD] transition-colors">
                                                        {service.label}
                                                    </span>
